...
var Twitter = module.exports = function (keys)  { 
    ...
    var backoffNetwork = new Backoff(new LinStrategy({
        initialDelay: 250,
        maxDelay: 16000
    }))
    ...
    this.connection.on('connection error known', this.emits('connection error known'));
    ...
    this.connection.on('connection error network', function (msg) {
        backoffNetwork.backoff();
    });

    this.connection.on('connection error known', function (statusCode) {
